...
String s;
    ...
    void getInfo(){
   String sql = "select * from users where userID='"+getDetails.userid+"'";
    ...
    void getPersonalInfo(){
     String sql = "select * from user_personal_info where userID='"+getDetails.userid+"'";
         Connection conn=SQLCon.ConnectDB();
         try{
            pst=conn.prepareStatement(sql);
            rs = pst.executeQuery();
...
void getContactInfo(){
     String sql = "select * from user_contact_info where userID='"+getDetails.userid+"'";
         Connection conn=SQLCon.ConnectDB();
         try{
            pst=conn.prepareStatement(sql);
            rs = pst.executeQuery();
...
void loadinfo(){
    String sql = "SELECT * FROM users WHERE userID=3";
         Connection conn=SQLCon.ConnectDB();
         try{
            pst=conn.prepareStatement(sql);
            rs = pst.executeQuery();
...
void updateMainInfo(){
     String sql = "update users set fName=?,mName=?,lName=?,gender=?,userType=?,username=?,password=? where userID='"+getDetails.userid+"'";
         Connection conn=SQLCon.ConnectDB();
         try{
            pst=conn.prepareStatement(sql);
                ...
                pst.setString(4, (String) cboGender.getSelectedItem());
                 pst.setString(5, (String) cboUserType.getSelectedItem());
                  pst.setString(6, txtUsernname.getText());
                   ...
                   pst.executeUpdate();
                   if (wImage.equals("No")) {
          String sqlNoImage = "UPDATE users SET image=? WHERE userID='"+getDetails.userid+"'";
          pst = conn.prepareStatement(sqlNoImage);
          pst.setString(1, "");
          pst.executeUpdate();
      } else if (wImage.equals("Yes")) {
          String sqlWithImage = "UPDATE users SET image=? WHERE userID='"+getDetails.userid+"'";
         ...
         void updatePersonalInfo(){
             ...
             try{
                 String sql = "update user_personal_info set birthDate='"+((JTextField)txtDOB.getDateEditor().getUiComponent()).getText()+"',bloodType='"+cboBloodType.getSelectedItem()+"' where userID='"+getDetails.userid+"'";
                 pst=conn.prepareStatement(sql);
              pst.executeUpdate();
    ...
    private void initComponents() {
        ...
        jButton4.setBounds(570, 90, 120, 40);
